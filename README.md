...
A monorepo of 12 dev-only packages for observing, debugging, and analyzing React component renders. Install the unified SDK for everything pre-wired, or pick individual packages for targeted use. Zero production cost —
all instrumentation is gated on `NODE_ENV !== 'production'`.

...
export const logCycle = (cycle: RenderCycle, mode: LogMode): void => {
	...
	if (mode === 'flat') {
		...
		if (rootCount > 1) {
			console.warn(`[render-trace] ${rootCount} root-depth components detected. ` + 'Some ancestors may not be instrumented.');
		}
	...
	if (tree.length === 0) {
		...
		if (tree.length > 1) {
			console.warn(`[render-trace] ${tree.length} disconnected render roots detected. ` + 'Some ancestors may not be instrumented.');
		}
	}

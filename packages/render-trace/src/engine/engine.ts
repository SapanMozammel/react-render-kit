		...
		registerNode: (componentName: string): void => {
			...
			if (stack.length > 0 && stack[stack.length - 1] === componentName) {
				if (!strictModeWarned) {
					...
					console.warn(`[render-trace] React Strict Mode detected — skipping duplicate registration of "${componentName}". Depth values in the first cycle may be approximate.`);
				}
				return;
			}
		...
		unregisterNode: (componentName: string): void => {
			...
			if (top !== componentName) {
				...
				if (!concurrentModeWarned) {
					...
					console.warn(`[render-trace] Render stack mismatch (Concurrent Mode artifact) — expected "${componentName}", found "${top}". In-flight cycle discarded.`);
				}
				stack.length = 0;
				activeCycle = null;
				flushPending = false;

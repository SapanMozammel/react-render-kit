...
describe('logCycle', () => {
	...
	describe('tree mode', () => {
		it('opens a console group with cycle id and root trigger', () => {
			...
			expect(groupSpy).toHaveBeenCalledWith(expect.stringContaining('cycle-1'));
		...
		it('warns on multiple disconnected roots (partial instrumentation)', () => {
			...
			logCycle(cycle, 'tree');
			expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('disconnected render roots'));
		});
	});
	...
	describe('flat mode', () => {
		...
		it('warns when multiple root-depth nodes are present', () => {
			...
			logCycle(cycle, 'flat');
			expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('ancestors may not be instrumented'));
		});
	});
});
